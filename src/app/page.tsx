...
export default function Dashboard() {
  ...
  async function deleteConversation() {
    if (!selected) return;
    const label = selected.name || selected.phone;
    if (!confirm(`Delete conversation with ${label}? All messages will be removed.`)) return;

    const res = await fetch(`/api/conversations/${selected.id}`, { method: "DELETE" });
    if (!res.ok) {
            ...
            <div className="px-6 py-4 border-b border-white/[0.06] flex items-center justify-between" style={{ background: "#141414" }}>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg text-xs font-medium text-red-400/90 hover:text-red-300 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 transition-all duration-200"
